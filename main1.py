...
def handle(msg):
	...
	if content_type == "text": #checks whether user entered text
		...
		messages = msg["text"] 
		chat_id= str(chat_id)
		if messages == "/summarise@SummarierBot" or messages == "/summarise": #checks whether user entered summarise command
			db.__init__("ChatDB")
			startSummarise(chat_id)
		elif messages == "/clear@SummarierBot" or messages == "/clear": #checks whether use entered clear command
			db.__init__("ChatDB")
